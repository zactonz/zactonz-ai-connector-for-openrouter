...
class OpenRouterSettings {
	...
	private const PAGE_SLUG        = 'zactonz-ai-provider-openrouter';
	...
	private static function model_capabilities(): array {
		$capabilities = array( 'text' => __( 'Default text model', 'zactonz-ai-provider-openrouter' ) );

		if ( OpenRouterProfile::supports( 'vision' ) ) {
			$capabilities['vision'] = __( 'Default vision model', 'zactonz-ai-provider-openrouter' );
		}
		if ( OpenRouterProfile::supports( 'image' ) ) {
			$capabilities['image'] = __( 'Default image model', 'zactonz-ai-provider-openrouter' );
		}
		if ( OpenRouterProfile::supports( 'embedding' ) ) {
			$capabilities['embedding'] = __( 'Default embedding model', 'zactonz-ai-provider-openrouter' );
		}
		if ( OpenRouterProfile::supports( 'tools' ) ) {
			$capabilities['tools'] = __( 'Default tool model', 'zactonz-ai-provider-openrouter' );
		}
	...
	public function register_settings(): void {
		...
		add_settings_section(
			self::SECTION_MAIN,
			__( 'Connection', 'zactonz-ai-provider-openrouter' ),
			'__return_empty_string',
			self::PAGE_SLUG
		);
		add_settings_section(
			self::SECTION_MODELS,
			__( 'Models', 'zactonz-ai-provider-openrouter' ),
			'__return_empty_string',
			self::PAGE_SLUG
		);
		add_settings_section(
			self::SECTION_ADVANCED,
			__( 'Advanced', 'zactonz-ai-provider-openrouter' ),
			'__return_empty_string',
			self::PAGE_SLUG
		);
		...
		add_settings_field(
			...
			__( 'API key', 'zactonz-ai-provider-openrouter' ),
		...
		add_settings_field(
			...
			__( 'Diagnostics', 'zactonz-ai-provider-openrouter' ),
		...
		if ( OpenRouterProfile::supports( 'reasoning' ) ) {
			add_settings_field(
				...
				__( 'Reasoning effort', 'zactonz-ai-provider-openrouter' ),
		...
		add_settings_field(
			...
			__( 'Available models', 'zactonz-ai-provider-openrouter' ),
		...
		add_settings_field(
			...
			__( 'API base URL', 'zactonz-ai-provider-openrouter' ),
		...
		add_settings_field(
			...
			__( 'Text request timeout', 'zactonz-ai-provider-openrouter' ),
			...
			array(
				...
				'help'      => __( 'Seconds to wait for a text response. Reasoning models may need longer.', 'zactonz-ai-provider-openrouter' ),
			)
		);
		...
		if ( OpenRouterProfile::supports( 'embedding' ) ) {
			add_settings_field(
				...
				__( 'Embedding request timeout', 'zactonz-ai-provider-openrouter' ),
				array( $this, 'render_timeout_field' ),
				self::PAGE_SLUG,
				self::SECTION_ADVANCED,
				array(
					...
					'help'      => __( 'Used for single and batch embedding requests.', 'zactonz-ai-provider-openrouter' ),
				)
			);
		}
		...
		if ( OpenRouterProfile::supports( 'image' ) ) {
			add_settings_field(
				...
				__( 'Image request timeout', 'zactonz-ai-provider-openrouter' ),
				array( $this, 'render_timeout_field' ),
				self::PAGE_SLUG,
				self::SECTION_ADVANCED,
				array(
					...
					'help'      => __( 'Seconds to wait for generated images.', 'zactonz-ai-provider-openrouter' ),
				)
			);
		}
	}
	...
	public function register_settings_screen(): void {
		add_options_page(
			...
			sprintf( __( '%s Settings', 'zactonz-ai-provider-openrouter' ), OpenRouterProfile::name() ),
				...
				if ( $seconds < $bounds[0] || $seconds > $bounds[1] ) {
					add_settings_error(
						...
						sprintf(
							...
							__( 'Use a timeout between %1$d and %2$d seconds.', 'zactonz-ai-provider-openrouter' ),
							$bounds[0],
							$bounds[1]
						)
					);
		...
		<div class="wrap zctz-openrouter-settings">
			...
			<p>
				...
				printf(
					...
					esc_html__( 'The API key can also be managed on the %1$sSettings > Connectors%2$s screen. This screen adds the %3$s options WordPress does not ask for.', 'zactonz-ai-provider-openrouter' ),
			...
			<p>
				...
				printf(
					...
					esc_html__( 'Create an API key on the %1$sprovider dashboard%2$s.', 'zactonz-ai-provider-openrouter' ),
	...
	public function render_api_key_field(): void {
		...
		<input
			...
			autocomplete="off"
			spellcheck="false"
			placeholder="<?php echo esc_attr( $has_key ? __( 'A key is saved and hidden', 'zactonz-ai-provider-openrouter' ) : __( 'Enter an API key', 'zactonz-ai-provider-openrouter' ) ); ?>"
		...
		<?php if ( $has_key && '' === $override ) : ?>
			<p>
				<label>
					...
					<?php echo esc_html__( 'Remove the saved API key', 'zactonz-ai-provider-openrouter' ); ?>
				</label>
			</p>
		<?php endif; ?>
		<p class="description">
			<?php if ( '' !== $override ) : ?>
				...
				printf(
					...
					esc_html__( 'The key is supplied by the %1$s%2$s%3$s constant or environment variable and cannot be edited here.', 'zactonz-ai-provider-openrouter' ),
					'<code>',
			...
			<?php else : ?>
				...
				printf(
					/* translators: 1: Opening code tag, 2: Constant name, 3: Closing code tag. */
					esc_html__( 'Leave blank to keep the saved key. Defining %1$s%2$s%3$s in wp-config.php overrides this field.', 'zactonz-ai-provider-openrouter' ),
					'<code>',
					esc_html( OpenRouterProfile::api_key_constant() ),
					'</code>'
				);
	...
	public function render_connection_field( array $args ): void {
		...
		if ( ! empty( $field['secret'] ) ) {
			...
			<input
				...
				autocomplete="off"
				spellcheck="false"
				placeholder="<?php echo esc_attr( $has_secret ? __( 'A value is saved and hidden', 'zactonz-ai-provider-openrouter' ) : (string) ( $field['placeholder'] ?? '' ) ); ?>"
			/>
			<?php if ( $has_secret ) : ?>
				<p>
					<label>
						<input type="checkbox" name="<?php echo esc_attr( self::OPTION_NAME . '[clear_' . $key . ']' ); ?>" value="1" />
						<?php echo esc_html__( 'Remove the saved value', 'zactonz-ai-provider-openrouter' ); ?>
					</label>
				</p>
			<?php endif; ?>
			<?php
	...
	public function render_base_url_field(): void {
		...
		/>
		<p class="description">
			<?php echo esc_html__( 'Leave blank for the default endpoint. Change this only for a proxy or a private gateway.', 'zactonz-ai-provider-openrouter' ); ?>
		</p>
	...
	public function render_model_field( array $args ): void {
		...
		>
			<option value=""><?php echo esc_html__( 'Automatic', 'zactonz-ai-provider-openrouter' ); ?></option>
			<?php if ( '' !== $value ) : ?>
				<option value="<?php echo esc_attr( $value ); ?>" selected><?php echo esc_html( $value ); ?></option>
			<?php endif; ?>
		</select>
		<p class="description">
			<?php echo esc_html__( 'Only models that report this capability are offered. Automatic lets the WordPress AI Client choose.', 'zactonz-ai-provider-openrouter' ); ?>
		</p>
	...
	public function render_reasoning_field(): void {
		...
		$labels = array(
			'default' => __( 'Model default', 'zactonz-ai-provider-openrouter' ),
			'none'    => __( 'No reasoning', 'zactonz-ai-provider-openrouter' ),
			'minimal' => __( 'Minimal', 'zactonz-ai-provider-openrouter' ),
			'low'     => __( 'Low', 'zactonz-ai-provider-openrouter' ),
			'medium'  => __( 'Medium', 'zactonz-ai-provider-openrouter' ),
			'high'    => __( 'High', 'zactonz-ai-provider-openrouter' ),
		);
		?>
		...
		<p class="description">
			<?php echo esc_html__( 'Applied to the default text model when it reports reasoning support. Models that always reason ignore this.', 'zactonz-ai-provider-openrouter' ); ?>
		</p>
	...
	public function render_timeout_field( array $args ): void {
		...
		<input
			...
			step="1"
		/>
		<span><?php echo esc_html__( 'seconds', 'zactonz-ai-provider-openrouter' ); ?></span>
	...
	public function render_diagnostics_field(): void {
		...
		<button type="button" class="button" id="zctz-openrouter-run-diagnostics">
			<?php echo esc_html__( 'Run diagnostics', 'zactonz-ai-provider-openrouter' ); ?>
		</button>
		...
		<p class="description">
			<?php echo esc_html__( 'Checks the endpoint, latency, discovered models, and the chosen defaults. Credentials are never displayed.', 'zactonz-ai-provider-openrouter' ); ?>
		</p>
	...
	public function enqueue_settings_assets( string $hook_suffix ): void {
		...
		wp_enqueue_script(
			'zactonz-ai-provider-openrouter-settings',
		...
		wp_localize_script(
			...
			array(
				...
				'strings'        => array(
					'loadingModels'   => __( 'Loading models...', 'zactonz-ai-provider-openrouter' ),
					'noModels'        => __( 'No models were returned for this API key.', 'zactonz-ai-provider-openrouter' ),
					'modelsFailed'    => __( 'Could not load models.', 'zactonz-ai-provider-openrouter' ),
					'automatic'       => __( 'Automatic', 'zactonz-ai-provider-openrouter' ),
					'running'         => __( 'Running diagnostics...', 'zactonz-ai-provider-openrouter' ),
					'connected'       => __( 'Connected', 'zactonz-ai-provider-openrouter' ),
					'notConnected'    => __( 'Not connected', 'zactonz-ai-provider-openrouter' ),
					'endpoint'        => __( 'Endpoint', 'zactonz-ai-provider-openrouter' ),
					'latency'         => __( 'Latency', 'zactonz-ai-provider-openrouter' ),
					'modelCount'      => __( 'Models discovered', 'zactonz-ai-provider-openrouter' ),
					'missingDefaults' => __( 'Defaults not found in the model list', 'zactonz-ai-provider-openrouter' ),
					'aiClient'        => __( 'AI Client version', 'zactonz-ai-provider-openrouter' ),
					'reasoningYes'    => __( 'This default model reports reasoning support.', 'zactonz-ai-provider-openrouter' ),
					'reasoningNo'     => __( 'This default model does not report reasoning support.', 'zactonz-ai-provider-openrouter' ),
					'reasoningPick'   => __( 'Choose a default text model to see whether it supports reasoning.', 'zactonz-ai-provider-openrouter' ),
					'context'         => __( 'context', 'zactonz-ai-provider-openrouter' ),
				),
			)
		);
	}
	...
	public function enqueue_connector_assets( string $hook_suffix ): void {
		...
		wp_enqueue_script(
			'zactonz-ai-provider-openrouter-connector',
		...
		wp_localize_script(
			...
			array(
				...
				'strings'      => array(
					...
					'save'         => __( 'Save and check connection', 'zactonz-ai-provider-openrouter' ),
					'saving'       => __( 'Checking...', 'zactonz-ai-provider-openrouter' ),
					'saved'        => __( 'Connection settings saved.', 'zactonz-ai-provider-openrouter' ),
					'unexpected'   => __( 'WordPress returned an unexpected response.', 'zactonz-ai-provider-openrouter' ),
					'moreSettings' => __( 'All connector settings', 'zactonz-ai-provider-openrouter' ),
					'savedValue'   => __( 'A value is saved and hidden', 'zactonz-ai-provider-openrouter' ),
				),
			)
		);
	}
	...
	public function ajax_list_models(): void {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( __( 'Insufficient permissions.', 'zactonz-ai-provider-openrouter' ), 403 );
		}
	...
	public function ajax_save_connection(): void {
		...
		wp_send_json_success(
			array(
				...
				'message'   => sprintf(
					/* translators: %s: Provider name. */
					__( '%s accepted these credentials.', 'zactonz-ai-provider-openrouter' ),
					OpenRouterProfile::name()
				),
			)
		);
	...
	public function get_models() {
		if ( ! class_exists( AiClient::class ) ) {
			return new WP_Error( 'ai_client_not_found', __( 'The WordPress AI Client is not available.', 'zactonz-ai-provider-openrouter' ) );
		}

		$registry = AiClient::defaultRegistry();
		if ( ! $registry->hasProvider( OpenRouterProfile::id() ) ) {
			return new WP_Error( 'ai_provider_not_found', __( 'The connector is not registered with the AI Client.', 'zactonz-ai-provider-openrouter' ) );
		}
		...
		if ( ! self::has_credentials() ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: Provider name. */
					__( 'Add the %s credentials before listing models.', 'zactonz-ai-provider-openrouter' ),
					OpenRouterProfile::name()
				)
			);
		}
		...
		try {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Could not list %1$s models. Error: %2$s', 'zactonz-ai-provider-openrouter' ),
					OpenRouterProfile::name(),
					$e->getMessage()
				)
			);
	...
	public static function verify_connection() {
		if ( ! self::has_credentials() ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: Provider name. */
					__( 'Add the %s credentials before checking the connection.', 'zactonz-ai-provider-openrouter' ),
					OpenRouterProfile::name()
				)
			);
		}
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: 1: Provider name, 2: Error message. */
					__( 'Could not reach %1$s. Error: %2$s', 'zactonz-ai-provider-openrouter' ),
					OpenRouterProfile::name(),
		...
		if ( 401 === $status || 403 === $status ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: Provider name. */
					__( '%s rejected these credentials.', 'zactonz-ai-provider-openrouter' ),
					OpenRouterProfile::name()
				)
			);
		}
		...
		if ( $status < 200 || $status >= 300 ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( '%1$s returned HTTP %2$d while listing models.', 'zactonz-ai-provider-openrouter' ),
					OpenRouterProfile::name(),
					$status
				)
			);
		...
		if ( ! is_array( $body ) || ( ! isset( $body['data'] ) && ! isset( $body['models'] ) && ! isset( $body['modelSummaries'] ) ) ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: Provider name. */
					__( '%s returned an unexpected model list.', 'zactonz-ai-provider-openrouter' ),
					OpenRouterProfile::name()
				)
			);
		}
