 ...
 * Plugin URI:        https://github.com/zactonz/zactonz-ai-provider-openrouter
 ...
 * Text Domain:       zactonz-ai-provider-openrouter
 *
...
define( 'ZCTZ_OPENROUTER_MIN_PHP_VERSION', '7.4' );
define( 'ZCTZ_OPENROUTER_MIN_WP_VERSION', '7.0' );
define( 'ZCTZ_OPENROUTER_VERSION', '1.0.0' );
define( 'ZCTZ_OPENROUTER_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
define( 'ZCTZ_OPENROUTER_PLUGIN_FILE', __FILE__ );

/**
 * Displays an admin notice for a requirement failure.
 *
 * @since 1.0.0
 *
...
/**
 * Checks whether the PHP version meets the minimum requirement.
 *
 * @since 1.0.0
 *
...
function zctz_check_php_version(): bool {
	if ( version_compare( phpversion(), ZCTZ_OPENROUTER_MIN_PHP_VERSION, '<' ) ) {
		add_action(
			'admin_notices',
			static function () {
				zctz_requirement_notice(
					sprintf(
						...
						__( 'The Zactonz AI Connector: OpenRouter requires PHP version %1$s or higher. You are running PHP version %2$s.', 'zactonz-ai-provider-openrouter' ),
...
/**
 * Checks whether the WordPress version meets the minimum requirement.
 *
 * @since 1.0.0
 *
...
function zctz_check_wp_version(): bool {
	if ( ! is_wp_version_compatible( ZCTZ_OPENROUTER_MIN_WP_VERSION ) ) {
		add_action(
			'admin_notices',
			static function () {
				global $wp_version;
				zctz_requirement_notice(
					sprintf(
						...
						__( 'The Zactonz AI Connector: OpenRouter requires WordPress version %1$s or higher. You are running WordPress version %2$s.', 'zactonz-ai-provider-openrouter' ),
...
function zctz_autoload( string $class_name ): void {
	...
	$base_dir       = ZCTZ_OPENROUTER_PLUGIN_DIR . 'includes/';
	$file_paths     = array(
